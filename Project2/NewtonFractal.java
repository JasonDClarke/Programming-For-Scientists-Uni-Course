...
class NewtonFractal {
    ...
    public Complex pixelToComplex(int i, int j) {
		return new Complex(origin.getReal()+(i*this.width/(double)NUMPIXELS)
                        ,origin.getImag()-(j*this.width/(double)NUMPIXELS));
    }
    ...
    public void createFractal(boolean colorIterations) {
        ...
        for (i=0;i<NUMPIXELS;i++)
        {
           ...
           {
               Complex x = pixelToComplex(i,j);
               itr = new Newton(iterator);
               itr.iterate(x);
               if (itr.getError()==-1)
               {
               root=0;
               iterations=Newton.MAXITER;
               }
               else if (itr.getError()==-2)
               {
               root=0;
               iterations=Newton.MAXITER;
               }
               else if (findRoot(itr.getRoot())==-1)
               {
               roots.add(itr.getRoot());
               root=findRoot(itr.getRoot());
               iterations=itr.getNumIterations();
               }
               else 
               {root=findRoot(itr.getRoot());
               iterations=itr.getNumIterations();
    ...
    public static void main(String[] args) {
        ...
        Complex[] coeff = new Complex[] { new Complex(-1.0,1.0), new Complex(-1.0,1.0), 
                                          new Complex(0.47,-0.5), new Complex(0.1,-0.5),
                                          new Complex(-1.0,01), new Complex(-0.5,0)};
